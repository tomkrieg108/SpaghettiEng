...
namespace MathX
{
  ...
  namespace AML
  {
    const Matrix33 DCM::rotationX(Real theta)
    {
        Real data[3][3] = { {1.0, 0.0,         0.0},
                              {0.0, cos(theta),  sin(theta)},
                              {0.0, -sin(theta), cos(theta)} };
        return Matrix33(data);
    }
    const Matrix33 DCM::rotationY(Real theta)
    {
        Real data[3][3] = { {cos(theta), 0.0, -sin(theta)},
                              {0.0,        1.0, 0.0},
                              {sin(theta), 0.0, cos(theta)} };
        return Matrix33(data);
    }
    const Matrix33 DCM::rotationZ(Real theta)
    {
        Real data[3][3] = { {cos(theta),  sin(theta), 0.0},
    ...
    bool isValidDCM(const Matrix33& dcm, Real tol)
    {
        Real tol_limit = 2.0 * tol;
    ...
    {
        const Real p = bodyRates.x;
        const Real q = bodyRates.y;
        const Real r = bodyRates.z;
        const Real skewMatrixData[9] = {0.0, -r, q, r, 0.0, -p, -q, p, 0.0};
    ...
    {
        const Real p = worldRates.x;
        const Real q = worldRates.y;
        const Real r = worldRates.z;
        const Real skewMatrixData[9] = {0.0, -r, q, r, 0.0, -p, -q, p, 0.0};
        const Matrix33 skewMatrix = Matrix33(skewMatrixData);
