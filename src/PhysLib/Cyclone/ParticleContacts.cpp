...
namespace Cyc
{
  ...
  void ParticleContact::ResolveVelocity(SpgMth::Real time_step) {
    ...
    SpgMth::Real separating_velocity = CalculateSeparatingVelocity();
    ...
    SpgMth::Real delta_velocity = new_sep_velocity - separating_velocity;
    
    // We apply the change in velocity to each object in proportion to
    // their inverse mass (i.e. those with lower inverse mass [higher
    // actual mass] get less change in velocity)..
    SpgMth::Real total_inverse_mass = particle[0]->GetInverseMass();
  ...
  void ParticleContact::ResolveInterpenetration(SpgMth::Real time_step) {
  ...
  void ParticleContactResolver::ResolveContacts(ParticleContact *contact_array, uint32_t num_contacts,
            SpgMth::Real time_step) {

    m_iterations_used = 0;
    while(m_iterations_used < m_iterations) {
      // Find the contact with the largest closing velocity;
      SpgMth::Real max = SpgMth::REAL_MAX;
      uint32_t max_index = num_contacts;
      for(auto i = 0; i< num_contacts; i++) {
        SpgMth::Real sep_vel = contact_array[i].CalculateSeparatingVelocity();
