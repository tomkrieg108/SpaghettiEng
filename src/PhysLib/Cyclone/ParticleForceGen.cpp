#include "PhysLib/Cyclone/ParticleForceGen.h"
...
namespace Cyc
{
  ...
  void ParticleForceRegistry::UpdateForces(SpgMth::Real time_step) {
    for (auto& r : m_registrations) {
      r.fg->UpdateForce(r.particle, time_step);
    }
  }

  void ParticleGravity::UpdateForce(Particle* particle, SpgMth::Real time_step)
  {
    if (!particle->HasFiniteMass()) 
      return;
    glm::vec3 force = m_gravity * particle->GetMass();
  ...
  void ParticleSpring::UpdateForce(Particle* particle, SpgMth::Real time_step)
  {
  ...
  void ParticleAnchoredSpring::UpdateForce(Particle* particle, SpgMth::Real time_step)
  {
    glm::vec3 force = particle->GetPosition();
    force -= *m_anchor;
    SpgMth::Real magnitude = glm::length(force);
  ...
  void ParticleFakeSpring::UpdateForce(Particle* particle, SpgMth::Real time_step)
  {
    ...
    glm::vec3 force = accel * particle->GetMass();     
  ...
  void ParticleAnchoredBungee::UpdateForce(Particle* particle, SpgMth::Real time_step)
  {
    glm::vec3 force = particle->GetPosition();
    force -= *m_anchor;
    SpgMth::Real magnitude = glm::length(force);
  ...
  void ParticleBuoyancy::UpdateForce(Particle* particle, SpgMth::Real time_step)
  {
    SpgMth::Real depth = particle->GetPosition().y;
